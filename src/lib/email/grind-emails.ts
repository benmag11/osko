...
import { GrindReminderEmail } from './templates/grind-reminder'
...
/**
 * Send a reminder email 2 hours before the grind starts
 */
export async function sendGrindReminderEmail(data: GrindEmailData): Promise<{ success: boolean; error?: string }> {
  try {
    const html = await render(
      GrindReminderEmail({
        userName: data.userName,
    ...
    const { error } = await resend.emails.send({
      from: `${FROM_NAME} <${FROM_EMAIL}>`,
      to: data.userEmail,
      subject: `Reminder: ${data.grindTitle} starts in 2 hours`,
      html,
      headers: {
        'X-Entity-Ref-ID': `grind-reminder/${data.grindId}/${data.userEmail}`,
      },
    })

    if (error) {
      console.error('Failed to send grind reminder email:', error)
      return { success: false, error: error.message }
    }

    return { success: true }
  } catch (err) {
    console.error('Error sending grind reminder email:', err)
    return { success: false, error: 'Failed to send email' }
  }
}
