...
import { getSubjectsWithAudioQuestionsClient } from '@/lib/supabase/audio-client-queries'
import { generateSlug } from '@/lib/utils/slug'
import { isLcvpSubject } from '@/lib/utils/points-calculator'
import type { Subject } from '@/lib/types/database'

interface AudioSubjectsPanelProps {
  currentSubject: Subject
}

/**
 * Subjects panel for audio sidebar
 * Only shows subjects that have audio questions and links to /audio/[slug]
 */
export function AudioSubjectsPanel({ currentSubject }: AudioSubjectsPanelProps) {
  const router = useRouter()

  // Fetch subjects that have audio questions
  const { data: subjects = [], isLoading } = useQuery({
    queryKey: ['audio-subjects'],
    queryFn: getSubjectsWithAudioQuestionsClient,
          ...
          {sortedSubjects.map((subject) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Icon className={cn('h-4 w-4', isActive ? 'text-white' : 'text-stone-600')} />
                </div>
