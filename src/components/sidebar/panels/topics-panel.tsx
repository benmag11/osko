...
/**
 * Topics panel with collapsible groups
 * Used by both normal and audio sidebars
 * Audio topics are converted to Topic[] format before being passed here
 */
...
function TopicCheckbox({ topic, checked, onToggle, disabled, indented = false }: TopicCheckboxProps) {
  return (
    <label
      className={cn(
    ...
    <Collapsible open={isExpanded} onOpenChange={onToggleExpand}>
      <CollapsibleTrigger asChild>
        ...
        >
          ...
          />
          {selectedCount > 0 && (
            <span className="ml-auto text-xs text-salmon-500 font-medium">{selectedCount}</span>
          )}
        </button>
      </CollapsibleTrigger>
      <CollapsibleContent>
